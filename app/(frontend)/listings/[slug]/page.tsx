...
import {PROPERTY_BY_SLUG, PROPERTY_SLUGS} from "@/sanity/queries/propertyQueries"
...
export async function generateStaticParams() {
  if (!client) {
    return dummyProperties.map((item) => ({
      slug: item.slug.current,
    }));
  }

  const posts = await client.fetch(PROPERTY_SLUGS);
...
export default async function Page({params}: {params: {slug: string}}) {
    ...
    let properties = dummyProperties.find((item)=> item.slug.current === params.slug )

    if(client){

        try {
            const sanityProperties = await client.fetch(PROPERTY_BY_SLUG, {slug: params.slug})
            properties = sanityProperties
        }
        catch(error){
            console.log("showing error from slug page", error)
        }
